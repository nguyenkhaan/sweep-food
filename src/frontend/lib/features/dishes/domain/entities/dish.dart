...
abstract class Dish with _$Dish {
  ...
  NutritionInfo get nutritionTotal => nutritionPerServing.scale(servings.toDouble());

  /// Non-seasoning ingredients (the checklist); seasonings render as chips.
  List<DishIngredient> get mainIngredients =>
      [for (final i in ingredients) if (!i.isSeasoning) i];
  ...
  String get metaLine {
    final parts = <String>['$prepTimeMin phút chuẩn bị'];
  ...
  String get shortMeta {
    final parts = <String>['$totalTimeMin phút'];
    if (difficulty.isNotEmpty) parts.add(difficulty);
    parts.add('${nutritionPerServing.energyKcal.round()} kcal / khẩu phần');
    return parts.join(' · ');
  }
