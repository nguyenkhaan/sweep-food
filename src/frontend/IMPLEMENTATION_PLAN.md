...
> **Còn treo cho M6:** bật lại `record` (thu âm thật) khi giải được lệch phiên bản `record_web`/`record_platform_interface` (cần mạng để resolve bộ tương thích) + macro `PERMISSION_MICROPHONE`/`PERMISSION_CAMERA` trong iOS Podfile cho `permission_handler`; bật lại crop I-02 = thêm `<activity com.yalantis.ucrop.UCropActivity>` vào AndroidManifest rồi đổi `capture(crop:true)` ở camera screen; ASR/OCR thật khi có backend (`scan_repository` + `MockApiClient` đã sẵn điểm swap).
